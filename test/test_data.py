...
class TestData(object):
    ...
    def test_get_idcard_checkcode(self):
        # id number <= 17
        id1 = '32012419870101'
        assert idcard_get_checkcode(id1)[0] is False
        id1 = '320124198701010012'
        assert idcard_get_checkcode(id1)[0] is False

        # id number checkcode
        id1 = '22068119870103456'
        assert idcard_get_checkcode(id1)[1] == '0'

        id1 = '62010519941220163'
        assert idcard_get_checkcode(id1)[1] != '2'  # is 9

    def test_is_valid_id_number(self):
        # id number false
        id1 = '320124198701010012'
        assert idcard_check_number(id1)[0] is False

        # id number true
        id2 = '130522198407316471'
        assert idcard_check_number(id2)[0] is True

        # id number irregularity
        id3 = '030522198407316471'
        assert idcard_check_number(id3)[0] is False

        # id number is int
        id3 = 130522198407316471
        assert idcard_check_number(id3)[0] is True

    # 2018.12.16 edit by David Yi
    def test_get_zonecode_by_area(self):
        # area_str，基本测试
        values = [('110000', '北京市')]
        assert idcard_get_zonecode_by_area('北京市') == values

        # area_str，显示设定参数
        result = idcard_get_zonecode_by_area(area_str='上海市')
        values = [('310000', '上海市')]
        assert result == values

        # area_str, match_type = EXACT 精确
        result = idcard_get_zonecode_by_area(area_str='北京市', match_type='EXACT')
        values = [('110000', '北京市')]
        assert result == values

        # area_str, match_type = FUZZY 模糊
        result = idcard_get_zonecode_by_area(area_str='北京市', match_type='FUZZY')
        ...
        result = idcard_get_zonecode_by_area(area_str='西安市', match_type='FUZZY', result_type='LIST')
        ...
        result = idcard_get_zonecode_by_area(area_str='北京市', match_type='EXACT', result_type='LIST')
        values = [('110000', '北京市')]
        assert result == values

        # area_str, match_type 精确, result_type=SINGLE_STR 字符串
        result = idcard_get_zonecode_by_area(area_str='北京市', match_type='EXACT', result_type='SINGLE_STR')
        values = '110000'
        assert result == values

        # area_str, 结果大于20个，返回20个
        result = len(idcard_get_zonecode_by_area(area_str='市', match_type='FUZZY'))
        assert result == 20

        # area_str, match_type 精确, result_type=SINGLE_STR 字符串, 无结果返回
        result = idcard_get_zonecode_by_area(area_str='美国', match_type='EXACT', result_type='SINGLE_STR')
        values = ''
        assert result == values
    ...
    def test_cardbin_get_cardbin_bank(self):
        ...
        result = cardbin_get_cardbin_bank('ICBC', 'CC')[0]
        assert result == values
        ...
        result = cardbin_get_cardbin_bank('CMB', 'CC')
        assert result == values
    ...
    def test_cardbin_get_checkcode(self):
        # 测试校验码不正确
        values = '0'
        result = cardbin_get_checkcode('439188000699010')
        assert result != values
    ...
    def test_cardbin_check_bankcard(self):
        # 测试银行卡校验码是否正确
        result = cardbin_check_bankcard('4391880006990100')
        assert result is False

        result = cardbin_check_bankcard('4391880006990109')
    ...
    def test_cardbin_get_bank_by_name(self):
        # 测试银行卡名称查询
        values = [('CMB', '招商银行')]
        result = cardbin_get_bank_by_name('招商银行')
        assert result == values

        # 测试银行卡名称查询
        values = 'HSB'
        result = cardbin_get_bank_by_name('恒生银行')
        assert result[0][0] == values

        # 测试不存在银行卡名称
        values = []
        result = cardbin_get_bank_by_name('招银行')
        assert result == values
